...
const Navbar = () => {
  ...
  const handleMenuItemClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string, type: string) => {
    setOpen(false);
  ...
  return (
    <nav className="bg-[#555454] px-4 py-6 flex items-center justify-between relative">
      <button
      ...
      {open && (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-0" onClick={() => setOpen(false)} />
      )}
    </nav>
  );
};
